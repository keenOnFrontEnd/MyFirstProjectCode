...
import { likeObject } from '../../../../redux/profile-reducer';
import s from './Post.module.css';


const Post = ({ message, likeObject }) => {
  let [likeCount, setLikeCount] = useState(0);
  let [DislikeCount, setDislikeCount] = useState(0);
  let [activeLike, setActiveLike] = useState(false);
  let [activeDisLike, setActiveDisLike] = useState(false);
  ...
  return (
    <div className={s.main_wrapper_post}>
      <div className={s.main_photo}><img alt='eee' 
      ...
      <div className={s.buttons} >
        <label className={activeDisLike ? s.active_dislike : s.inactive_dislike } onClick={() => dislikePost()}>
          <button className={s.dislike}  onClick={() => {
          dislikePost()
        ...
        <label className={activeLike? s.active_like : s.inactive_like } onClick={() => likePost()}>
        ...
        <label>
          <button className={s.repost}>repost</button> 
          Repost
          </label>
      </div>
      </div>
...
export const PostConnectedCont = connect(null, { likeObject })(Post)